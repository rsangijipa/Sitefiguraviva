...
jest.mock("@/components/sections/CoursesSection", () => {
  function MockCoursesSection({
    ...
    return (
      <section>
        {courses.map((course) => (
          <article data-testid="formation-card" key={course.id}>
            {course.title}
          </article>
        ))}
      </section>
...
jest.mock("@/components/sections/BlogSection", () => {
  function MockBlogSection({
    blogPosts = [],
  }: {
    blogPosts?: Array<{ id: string; title: string }>;
  }) {
    return (
      <section>
        {blogPosts.slice(0, 3).map((post) => (
          <article data-testid="content-card" key={post.id}>
            {post.title}
          </article>
        ))}
      </section>
