...
export function WindParticles({
  ...
  useFrame((state, delta) => {
    ...
    opacityRef.current +=
      (targetOpacity - opacityRef.current) * Math.min(1, delta * 2.4);
    ...
    const gust =
      0.68 + Math.sin(time * 0.27) * 0.22 + Math.sin(time * 0.61) * 0.1;
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      array[offset + 2] +=
        (WIND.z * speed + Math.cos(time * 0.95 + phase * 1.7) * 0.22 * sway) *
        step;
