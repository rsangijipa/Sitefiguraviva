...
export function generateTree(options: GenerateTreeOptions): TreeData {
  ...
  function emitLeaves(
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const position = point
        .clone()
        .addScaledVector(radial, attachRadius * 0.9);
      ...
      normal
        .normalize()
        .applyAxisAngle(blade, (random() - 0.5) * 1.15)
        .normalize();
  ...
  function growAxis(
    ...
    const lateralCount =
      level.lateralsMin +
      Math.floor(random() * (level.lateralsMax - level.lateralsMin + 1));
    const side = new THREE.Vector3();
    const upSide = new THREE.Vector3();
    ...
    for (let index = 0; index < lateralCount; index += 1) {
      ...
      const angle =
        level.angleMin + random() * (level.angleMax - level.angleMin);
      ...
      const childRadius =
        attachRadius * level.radiusRatio * (0.86 + random() * 0.26);

      // laterais mais baixos no eixo sao mais longos -> copa arredondada
      const childLength =
        length * level.lengthRatio * (0.8 + random() * 0.4) * (1.14 - t * 0.36);

      // nasce ligeiramente dentro do pai para a juncao nao ficar vazada
      const childOrigin = attachPoint
        .clone()
        .addScaledVector(childDirection, -attachRadius * 0.9);
  ...
  // --------------------------------------------------------------- tronco
  const trunkLean = new THREE.Vector3(
    (random() - 0.5) * 0.09,
    1,
    (random() - 0.5) * 0.09,
  ).normalize();
  ...
  for (let index = 0; index < rootCount; index += 1) {
    ...
    const start = new THREE.Vector3(
      0,
      options.trunkRadius * (0.85 + random() * 0.6),
      0,
    );
  ...
  for (const leaf of rawLeaves) {
    const horizontal = Math.hypot(
      leaf.position.x - crownCenter.x,
      leaf.position.z - crownCenter.z,
    );
    const vertical = THREE.MathUtils.clamp(
      (leaf.position.y - crownCenter.y) / verticalSpan,
      -1,
      1,
    );
    leaf.exposure = THREE.MathUtils.clamp(
      (horizontal / Math.max(0.4, crownRadius)) * 0.62 +
        (vertical * 0.5 + 0.5) * 0.38,
      0,
      1,
    );
  }
  ...
  const messageLeaves = pickMessageLeaves(
    rawLeaves,
    options.messageLeafCount,
    random,
  );
  const messageSources = new Set(messageLeaves.map((leaf) => leaf.position));

  const commonPool = rawLeaves.filter(
    (leaf) => !messageSources.has(leaf.position),
  );
...
function pickMessageLeaves(
  ...
  for (const leaf of pool) {
    ...
    const sector = Math.min(
      count - 1,
      Math.floor(((angle + Math.PI) / (Math.PI * 2)) * count),
    );
    const score =
      leaf.exposure * 0.75 + leaf.position.y * 0.06 + random() * 0.22;

    if (
      !bestBySector.has(sector) ||
      score > (bestScore.get(sector) as number)
    ) {
