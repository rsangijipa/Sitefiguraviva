...
export const LEAF_MIDRIB =
  "M219 359C515 358 819 366 1096 376C1266 381 1408 386 1527 394";
...
function buildDetailSvg() {
  const veins = [...LEAF_VEINS_UPPER, ...LEAF_VEINS_LOWER]
    .map(
      ([path, width]) =>
        `<path d="${path}" stroke="#6E655A" stroke-width="${width * 1.5}"/>`,
    )
    .join("");
 ...
 */
export function createLeafDetailTexture(
  resolution = 512,
...
export function messageLeafTone(index: number) {
  return MESSAGE_LEAF_TONES[
    ((index % MESSAGE_LEAF_TONES.length) + MESSAGE_LEAF_TONES.length) %
      MESSAGE_LEAF_TONES.length
  ];
}
