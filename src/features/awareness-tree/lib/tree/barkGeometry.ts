...
import {
  radiusAtParam,
  type BranchSegment,
} from "@/features/awareness-tree/lib/tree/generateTree";
...
function buildTube(
  ...
  const tubularBase = THREE.MathUtils.clamp(
    Math.round(curveLength * (isTrunk ? 12 : 9)),
    3,
    isTrunk ? 26 : 14,
  );
  ...
  for (let i = 0; i <= tubularSegments; i += 1) {
    ...
    for (let j = 0; j <= radialSegments; j += 1) {
      ...
      if (depth <= 1) {
        ...
        color
          .copy(WOOD_MID)
          .lerp(WOOD_YOUNG, THREE.MathUtils.clamp((depth - 1) / 3, 0, 1));
      }

      // luz batendo nas cristas da casca, sombra nos sulcos
      const grooveShade = thick
        ? THREE.MathUtils.clamp(groove / (radius * 0.05 + 1e-5), -1, 1)
        : 0;
      ...
      const mossAmount = isRoot
        ? 0.4
        : THREE.MathUtils.clamp(1 - worldY / 0.55, 0, 1) * 0.34;
      if (mossAmount > 0) {
  ...
  if (isTrunk || isRoot) {
    ...
    const ringStart =
      vertexOffset + (isTrunk ? 0 : (radialSegments + 1) * tubularSegments);
...
export function buildBarkGeometry(
  ...
  geometry.setAttribute(
    "color",
    new THREE.Float32BufferAttribute(target.colors, 3),
  );
