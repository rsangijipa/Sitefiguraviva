...
function veinMask(u: number, s: number) {
  ...
  const lateral =
    Math.pow(Math.max(0, Math.cos(phase * Math.PI)), 24) *
    (1 - absU) *
    Math.sin(Math.PI * s);
...
export function createLeafGeometry(
  ...
  for (let j = 0; j <= segmentsV; j += 1) {
    ...
    for (let i = 0; i <= segmentsU; i += 1) {
      ...
      const fold =
        -cup * Math.pow(absU, 1.7) * Math.sin(Math.PI * Math.max(s, 0.001));
      const ridge =
        midrib * Math.pow(1 - absU, 3) * Math.sin(Math.PI * Math.max(s, 0.001));
