...
import { Folder, FileText, Download, Search, Image as ImageIcon, Video, ArrowLeft } from 'lucide-react';
...
export default function StudentPortal() {
    ...
    const folders = [
        ...
        { id: 'portal', label: 'Portal Aluno' }, // Placeholder for when files are added
    ];
                ...
                <div className="bg-white rounded-3xl md:rounded-[2.5rem] shadow-[0_60px_100px_-20px_rgba(38,58,58,0.1)] overflow-hidden border border-primary/5 min-h-[700px] flex flex-col md:flex-row glass">
                    ...
                    <section className="flex-1 p-6 md:p-16">
                        <div className="flex justify-between items-center mb-12">
                            ...
                            <div className="hidden md:flex items-center gap-2 text-[10px] font-bold text-sage uppercase tracking-widest">
                                ...
                                {filteredResources.map(file => (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-6">
                                            <div className={`p-4 rounded-2xl shadow-sm ${file.type === 'pdf' ? 'bg-red-50 text-red-500' : 'bg-sage/10 text-sage'}`}>
                                                <FileText size={24} />
                                            </div>
                                            <a
                                                href={file.path}
                                                ...
                                                className="text-primary/20 hover:text-accent transition-colors p-2 glass rounded-full opacity-0 group-hover:opacity-100 transition-soft"
                                        ...
                                        <h3 className="font-bold text-primary text-sm mb-2 line-clamp-2 leading-relaxed group-hover:text-accent transition-colors" title={file.name}>
                                            {file.title || file.name}
                                        </h3>
                                        <div className="flex items-center gap-2 mt-4">
                                            <span className="text-[9px] font-bold text-primary/30 uppercase tracking-widest">{file.size}</span>
                                            <span className="w-1 h-1 rounded-full bg-primary/20" />
                                            <span className="text-[9px] font-bold text-primary/30 uppercase tracking-widest">PDF</span>
                                        </div>
                                    </motion.div>
                                ))}
                            </div>
