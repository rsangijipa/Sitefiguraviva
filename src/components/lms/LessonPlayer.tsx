...
export function LessonPlayer({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await getLessonContentAction(
          course.id,
          resolvedModuleId,
          activeLesson.id,
        );
        ...
        if (res?.success) {
          let rawBlocks = res.data?.blocks;
          if (
            rawBlocks &&
            typeof rawBlocks === "object" &&
            !Array.isArray(rawBlocks)
          ) {
            rawBlocks = Object.values(rawBlocks);
          }
          const serverBlocks = (
      ...
      <header className="h-16 bg-white border-b border-stone-100 flex items-center justify-between px-4 shrink-0 z-20 relative shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            leftIcon={
              activeLesson?.isCompleted ? <CheckCircle size={14} /> : undefined
            }
          >
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <aside
          className={cn(
            ...
            sidebarOpen
              ? "translate-x-0"
              : "translate-x-full md:w-0 overflow-hidden border-none",
          )}
        >
          <LessonSidebar
            modules={modules}
