...
import { CheckCircle, Loader2, Play, RotateCcw, AlertTriangle, Info } from 'lucide-react';
...
export function VideoPlayer({
    ...
    const [maxWatched, setMaxWatched] = useState(Math.max(initialMaxWatchedSecond, 0));
    ...
    const markAsCompleted = async () => {
        ...
        try {
            await updateLessonProgress(
                ...
                moduleId,
                lessonId,
                { status: 'completed', percent: 100, maxWatchedSecond: playerRef.current?.getDuration() || 0 }
            );
            setIsCompleted(true);
    ...
    return (
        <div className="space-y-4">
                    ...
                    <div className="absolute top-4 left-4 z-20 animate-in fade-in slide-in-from-top-4 duration-700 pointer-events-none">
                        <div className="bg-black/70 backdrop-blur-md text-white px-4 py-2 rounded-full text-sm flex items-center gap-3 shadow-xl pointer-events-auto">
                            <span className="flex items-center gap-2">
                                ...
                                Retomando de <span className="font-mono font-bold text-primary">{formatTime(resumeSeconds)}</span>
                            </span>
            ...
            <div className={cn(
                ...
                isCompleted ? "bg-green-50 border-green-200" : "bg-white border-stone-200"
            )}>
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className={cn("font-bold text-sm", isCompleted ? "text-green-800" : "text-stone-700")}>
                            {isCompleted ? "Aula Concluída" : "Progresso do Vídeo"}
                        </p>
                        {isCompleted ? (
                            <p className="text-xs text-green-600">Progresso salvo automaticamente.</p>
                        ) : (
                            <p className="text-xs text-stone-500">
                                {maxWatched > 0 ? `Assistido até ${formatTime(maxWatched)}` : 'Assista para contabilizar o progresso.'}
                            </p>
                        )}
                    </div>
                </div>
