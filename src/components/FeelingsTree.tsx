...
const TreeVisualization = dynamic(() => import("./TreeVisualization"), {
  ssr: false,
});
...
interface FeelingsTreeProps {
  isModal?: boolean;
  onClose?: () => void;
  userLevel?: number;
  userXP?: number;
  unlockedLeavesCount?: number;
  onLeafDiscovered?: (xpGained: number) => void;
}
...
export default function FeelingsTree({
  ...
  userLevel = 1,
  userXP = 0,
  unlockedLeavesCount = 0,
  onLeafDiscovered,
}: FeelingsTreeProps) {
  const [viewState, setViewState] = useState<"intro" | "tree" | "message">(
    "intro",
  );
  ...
  const handleLeafClick = (leafData: Leaf) => {
    ...
    if (onLeafDiscovered) {
      onLeafDiscovered(10); // +10 XP per leaf
    }
  };
  ...
  const StepIndicator = ({ step }: { step: number }) => (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 z-40">
      {[1, 2, 3].map((s) => (
        <div
          key={s}
          className={`h-1.5 rounded-full transition-all duration-500 ${s <= step ? "w-6 bg-stone-500/80 shadow-sm" : "w-2 bg-stone-300/50"}`}
        />
      ))}
    </div>
  );
  ...
  const renderIntro = () => (
    ...
    >
      ...
      <div className="bg-stone-100 p-6 rounded-full mb-8 relative">
        <Sparkles size={48} className="text-stone-600" />
        {userLevel > 1 && (
          <div className="absolute -top-2 -right-2 bg-gold text-white text-xs font-bold rounded-full w-8 h-8 flex items-center justify-center shadow-md border-2 border-[#f2f0e9]">
            L{userLevel}
          </div>
        )}
      </div>
      ...
      <div className="bg-white/50 border border-white p-4 rounded-2xl mb-8 w-full max-w-sm shadow-sm">
        <div className="flex justify-between items-center mb-2 text-sm font-bold text-stone-700">
          <span>Folhas Coletadas</span>
          <span className="text-primary">
            {unlockedLeavesCount} / {NUM_LEAVES}
          </span>
        </div>
        <div className="h-2 w-full bg-stone-200 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-primary to-gold transition-all duration-1000"
            style={{ width: `${(unlockedLeavesCount / NUM_LEAVES) * 100}%` }}
          />
        </div>
        <p className="text-xs text-stone-500 mt-3 font-semibold text-left">
          {" "}
          XP Atual: {userXP}
        </p>
      </div>
      ...
      <p className="max-w-md text-lg text-stone-600 mb-10 leading-relaxed font-light">
        Nesta árvore, cada folha guarda uma mensagem de sabedoria gestáltica.
        Conecte-se com sua intuição e escolha uma.
      </p>
      <button
    ...
    >
      ...
      <div className="absolute top-16 left-0 right-0 text-center z-20 pointer-events-none">
        ...
        <p className="text-sm text-stone-400">
          Gire a árvore e clique na folha que te chamar.
        </p>
      </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Side Veins Upper */}
          <path
            d="M150 190 C 160 150, 180 120, 220 80"
            stroke="rgba(255,255,255,0.15)"
            strokeWidth="2"
            fill="none"
          />
          <path
            d="M280 190 C 290 150, 310 120, 350 80"
            stroke="rgba(255,255,255,0.15)"
            strokeWidth="2"
            fill="none"
          />
          <path
            d="M410 190 C 420 160, 430 140, 460 110"
            stroke="rgba(255,255,255,0.15)"
            strokeWidth="2"
            fill="none"
          />
          ...
          {/* Side Veins Lower */}
          <path
            d="M150 190 C 160 230, 180 260, 220 300"
            stroke="rgba(255,255,255,0.15)"
            strokeWidth="2"
            fill="none"
          />
          <path
            d="M280 190 C 290 230, 310 260, 350 300"
            stroke="rgba(255,255,255,0.15)"
            strokeWidth="2"
            fill="none"
          />
          <path
            d="M410 190 C 420 220, 430 240, 460 270"
            stroke="rgba(255,255,255,0.15)"
            strokeWidth="2"
            fill="none"
          />
