...
const TreeVisualization = ({ emotions = [], onLeafClick, isModal = false }) => {
    ...
    try {
      ...
      const growBranch = (startPos, length, radius, depth, rotation) => {
        ...
        for (let i = 0; i < positionAttribute.count; i++) {
          ...
          if (y > 0 && y < length) {
            // Don't jitter base/top too much to keep connections
            const amp = 0.1 * radius;
            positionAttribute.setX(
              i,
              positionAttribute.getX(i) + (Math.random() - 0.5) * amp,
            );
            positionAttribute.setZ(
              i,
              positionAttribute.getZ(i) + (Math.random() - 0.5) * amp,
            );
          }
        }
      ...
      const onClick = (event) => {
        ...
        const intersects = raycaster.current.intersectObjects(
          leavesGroupRef.current.children,
        );
        ...
        if (cameraRef.current && leavesGroupRef.current) {
          ...
          leavesGroupRef.current.children.forEach((child, i) => {
            ...
            const targetRotation = isHovered
              ? child.userData.baseRotation + 0.5
              : child.userData.baseRotation;
            child.material.rotation +=
              (targetRotation - child.material.rotation) * 0.1;
          });
      ...
      /*
       * Resize: apenas reconfigura camera/renderer existentes — o
       * renderer NUNCA é recriado. Um ResizeObserver acompanha o
       * container (que dentro do ResourceExperienceShell muda de
       * tamanho sem que a janela mude), somado a window resize e
       * orientationchange para o caso mobile.
       */
      const handleResize = () => {
      ...
      return () => {
        ...
        if (containerNode && renderer.domElement.parentNode === containerNode) {
  ...
  // Dentro do shell o container preenche a área disponível (100%); fora
  // dele mantém a altura mínima original. Nada de 100vh, que estourava a
  // janela do recurso e criava um segundo scroll container.
  return (
    <div
      ref={containerRef}
      style={{
