    ...
    caches
      ...
      .then((keys) =>
        Promise.all(
          keys
            .filter(
              (key) => ![SHELL_CACHE, RUNTIME_CACHE, API_CACHE].includes(key),
            )
...
function isAssetRequest(url, destination) {
  ...
  if (
    ["script", "style", "font", "image", "audio", "video"].includes(destination)
  ) {
    return true;
  }

  return /\.(?:glb|gltf|bin|ktx2|basis|png|jpe?g|webp|avif|hdr|exr)$/i.test(
    url.pathname,
  );
}
