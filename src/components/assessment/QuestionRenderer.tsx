...
export const QuestionRenderer = ({
  ...
  const handleSingleSelect = (optionId: string | boolean) => {
    if (readOnly) return;
    onChange(optionId);
  };
  ...
  const renderChoice = () => {
    ...
    const isMulti = mcq.allowMultiple;
  ...
  const renderText = () => {
    ...
    // Note: EssayQuestion type has minLength/maxLength if we want to enforce it visually.
    // Assuming custom prop was intended or check question properties more strictly if needed.
    // For now using safe access or any cast if needed, but let's stick to base props.
    const minLength = (question as any).minLength;
    ...
    return (
      <div className="space-y-2">
        <textarea
          ...
          rows={6}
          placeholder={
            (question as any).placeholder || "Digite sua resposta aqui..."
          }
        ...
        {minLength && (
          <p className="text-xs text-stone-400 text-right">
            {(value as string)?.length || 0} / {minLength} caracteres mínimos
          </p>
        )}
      </div>
    );
    ...
    <div className="animate-fade-in">
      ...
      <div className="mb-6">
        <span className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-2 block">
          Questão {question.type === "essay" ? "Discursiva" : "Objetiva"} •{" "}
      ...
      {showFeedback && (
        <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-xl text-sm text-blue-800">
          ...
          <p>
            {QuestionRenderer.name} - Feedback integration pending grading
            result.
          </p>
        </div>
      )}
