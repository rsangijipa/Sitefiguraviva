...
export const AssessmentPlayer = ({ assessment, existingSubmission, onExit }: AssessmentPlayerProps) => {

    // Initial State Setup
    const [state, dispatch] = useReducer(reducer, {
        currentIndex: 0,
        answers: existingSubmission?.answers
            ? Object.entries(existingSubmission.answers).reduce((acc, [k, v]) => ({ ...acc, [k]: v.value }), {})
            : {},
        isSaving: false,
    ...
    const handleAnswerChange = (val: any) => {
        dispatch({ type: 'SET_ANSWER', questionId: currentQuestion.id, value: val });
    };

    const handleFinish = async () => {
        const confirm = window.confirm("Tem certeza que deseja finalizar a prova? Você não poderá alterar suas respostas depois.");
        if (confirm) {
            try {
        ...
        <div className="min-h-screen bg-[#FDFCF9] flex flex-col pb-24">
            ...
            <header className="h-16 px-6 bg-white border-b border-stone-100 flex items-center justify-between sticky top-0 z-40">
                <div className="flex items-center gap-4">
                    <h1 className="font-serif font-bold text-stone-800 text-lg line-clamp-1">{assessment.title}</h1>
                    {/* Autosave Indicator */}
