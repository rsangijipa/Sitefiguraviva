...
import { X, BookOpen, Image as ImageIcon, Save, Loader2, ChevronDown, Plus, Trash2 } from 'lucide-react';
...
export default function CourseEditorModal({ isOpen, onClose, onSuccess, editingCourse }: CourseEditorModalProps) {
    ...
    const [activeSection, setActiveSection] = useState<'info' | 'details' | 'settings'>('info');
        ...
        try {
            if (isEditing) {
                ...
                if (!result || !result.success) {
                    const errorMessage = (result as any)?.error || 'Erro desconhecido ao criar curso';
        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    >
                        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-full overflow-hidden pointer-events-auto flex flex-col">
                            {/* Header */}
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-white/70">
                                            {isEditing ? 'Atualize as informações do curso' : 'Preencha as informações do curso'}
                                        </p>
                                    </div>
                                </div>
                                <button
                            ...
                            <div className="flex border-b border-stone-100 bg-stone-50">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 px-6 py-4 text-sm font-medium transition-colors ${activeSection === section.id
                                    ...
                                    <div className="space-y-6 animate-fade-in">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="grid grid-cols-3 gap-4">
                                                {[
                                                    { value: 'draft', label: 'Rascunho', desc: 'Não visível para alunos', color: 'stone' },
                                                    { value: 'open', label: 'Publicado', desc: 'Aberto para inscrições', color: 'green' },
                                                    { value: 'closed', label: 'Encerrado', desc: 'Inscrições fechadas', color: 'orange' }
                                                ].map((option) => (
                                                    <button
                                                        key={option.value}
                                                        onClick={() => updateField('status', option.value)}
                                                        className={`p-4 rounded-xl border-2 text-left transition-all ${formData.status === option.value
                                                            ? option.color === 'green'
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-2">
                                                            <div className={`w-3 h-3 rounded-full ${option.color === 'green' ? 'bg-green-500' :
                                                                option.color === 'orange' ? 'bg-orange-500' : 'bg-stone-400'
                                                                }`} />
                                                            <span className="font-bold text-stone-800">{option.label}</span>
                                                        </div>
                                        ...
                                        <div className="bg-primary/5 border border-primary/20 rounded-xl p-6">
                                            <h4 className="font-bold text-primary mb-2">📚 Próximos Passos</h4>
                                            <ul className="text-sm text-stone-600 space-y-2">
                                                <li>• Após criar o curso, você poderá adicionar módulos e aulas</li>
