...
export default function CalendarModal({
  ...
  const getAllEvents = () => {
    ...
    courses.forEach((course) => {
      ...
      const yearContext =
        course.date && course.date.includes("2025") ? 2025 : 2026;
  ...
  const headerTitle = (
    <div>
      <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-gold block mb-2">
        Agenda 2026
      </span>
          ...
          {events.length > 0 ? (
            <div className="space-y-4">
              {events.map((evt, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-4 p-4 rounded-xl border border-gray-100 hover:border-gold/30 hover:bg-gold/5 transition-colors group bg-white"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-1">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-primary/40 bg-gray-50 px-2 py-0.5 rounded-full w-fit">
                        {evt.type}
                      </span>
                      <span className="text-[10px] font-bold text-gray-300 hidden md:block capitalize">
                        {evt.monthName}
                      </span>
                    </div>
                    <h4 className="font-serif text-lg text-primary leading-tight mb-2 group-hover:text-gold transition-colors">
                      {evt.title}
                    </h4>
                  ...
                  {evt.link && (
                    <a
                      ...
                      className="self-center p-2 text-primary/20 hover:text-gold transition-colors"
                    >
