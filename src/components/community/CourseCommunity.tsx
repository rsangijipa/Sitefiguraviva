...
export default function CourseCommunity({ courseId, user }: CourseCommunityProps) {
    const [view, setView] = useState<'list' | 'detail' | 'create'>('list');
    const [selectedThread, setSelectedThread] = useState<CommunityThreadDoc | null>(null);
    ...
    const handleCreateThread = async (e: React.FormEvent) => {
        ...
        setIsCreating(true);
        try {
            const id = await communityService.createThread(courseId, user, newThreadTitle, newThreadContent);
    ...
    if (view === 'detail' && selectedThread) {
        return <ThreadView courseId={courseId} thread={selectedThread} user={user} onBack={handleBack} />;
    }
    ...
    if (view === 'create') {
        return (
            <div className="animate-in fade-in max-w-2xl mx-auto">
                ...
                <form onSubmit={handleCreateThread} className="space-y-6 bg-white p-6 rounded-xl border border-stone-200 shadow-sm">
                    <div>
                        <label className="block text-sm font-bold text-stone-700 mb-2">Título</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label className="block text-sm font-bold text-stone-700 mb-2">Conteúdo</label>
                        <textarea
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h3 className="font-bold text-stone-800 text-lg">Discussões da Turma</h3>
                <Button size="sm" onClick={() => setView('create')}>Nova Pergunta</Button>
            </div>

            {loading ? (
                <div className="text-center py-12 text-stone-400">Carregando discussões...</div>
            ) : threads.length > 0 ? (
                    ...
                    {threads.map(thread => (
                        ...
                        >
                            <div className="flex justify-between items-start mb-2">
                                <h4 className="font-bold text-stone-700 group-hover:text-primary transition-colors text-base">{thread.title}</h4>
                            ...
                            <p className="text-sm text-stone-500 line-clamp-2 mb-4">{thread.content}</p>
                            <div className="flex items-center justify-between text-xs text-stone-400 font-medium border-t border-stone-50 pt-4">
                                ...
                                <span>{thread.lastReplyAt?.toDate ? `Atualizado em ${thread.lastReplyAt.toDate().toLocaleDateString()}` : ''}</span>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="text-center py-12 bg-white rounded-xl border border-dashed border-stone-200">
                    ...
                    <h3 className="font-bold text-stone-800 text-lg mb-2">Comunidade Vazia</h3>
                    <p className="text-stone-500 max-w-md mx-auto mb-6">Seja o primeiro a iniciar uma discussão!</p>
