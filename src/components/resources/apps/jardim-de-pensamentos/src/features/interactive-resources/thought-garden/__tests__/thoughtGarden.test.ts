...
async function runTests() {
  ...
  assert(!validateThoughtText('    ').isValid, 'Texto com apenas espaços deve ser rejeitado');
  assert(validateThoughtText('Uma respiração suave.').isValid, 'Texto curto válido deve ser aceito');
  assert(validateThoughtText('   Espaços ao redor   ').data === 'Espaços ao redor', 'Deve aparar espaços ao redor');
  
  const text501 = 'a'.repeat(501);
  assert(!validateThoughtText(text501).isValid, 'Texto com 501 caracteres deve ser rejeitado');

  const text500 = 'a'.repeat(500);
  assert(validateThoughtText(text500).isValid, 'Texto com exatamente 500 caracteres deve ser aceito');

  // 2. Validação de Título Opcional
  console.log('\n2. Testando título opcional...');
  assert(validateOptionalTitle(null).isValid && validateOptionalTitle(null).data === null, 'Título nulo é aceito');
  assert(validateOptionalTitle('   ').isValid && validateOptionalTitle('   ').data === null, 'Título vazio é aceito');
  assert(!validateOptionalTitle('t'.repeat(81)).isValid, 'Título com 81 caracteres deve ser rejeitado');
  assert(validateOptionalTitle('Título Válido').data === 'Título Válido', 'Título válido deve ser aceito');

  // 3. Sanitização contra injeção de fórmulas no CSV
  console.log('\n3. Testando proteção contra injeção de fórmulas no CSV...');
  assert(sanitizeCsvCell('=SUM(A1:A10)') === `"'=SUM(A1:A10)"`, 'Início com = deve ser prefixado com apóstrofo');
  assert(sanitizeCsvCell('+cmd|/c') === ` "'+cmd|/c"`.trim(), 'Início com + deve ser prefixado com apóstrofo');
  assert(sanitizeCsvCell('-123') === ` "'-123"`.trim(), 'Início com - deve ser prefixado com apóstrofo');
  assert(sanitizeCsvCell('@test') === ` "'@test"`.trim(), 'Início com @ deve ser prefixado com apóstrofo');
  assert(sanitizeCsvCell('Pensamento comum') === `"Pensamento comum"`, 'Texto normal não recebe apóstrofo');
  ...
  assert(state.experienceState === 'empty', 'Sem folhas, início vai para empty');

  state = thoughtGardenReducer(state, {
    type: 'PLACE_LEAF',
    payload: { text: 'Primeira folha no canteiro' },
  });
  assert(state.experienceState === 'active', 'Com folhas, estado torna-se active');
  assert(state.leaves.length === 1, 'Folha deve ser adicionada à cena');
  assert(state.leaves[0].status === 'placed', 'Status inicial da folha deve ser placed');
  ...
  assert(state.leaves[0].isFloating === false, 'Folha deve ter pousado de volta');
  ...
  assert(state.leaves.length === 1, 'Apenas a folha guardada deve permanecer no encerramento');
  assert(state.leaves[0].id === leafId, 'A folha guardada correta foi preservada');
  ...
  assert(thoughtsA[0].text === 'Anotação íntima do Aluno A', 'Conteúdo de A corresponde');

  const thoughtsB = await listUserThoughts(userB);
  assert(thoughtsB.length === 0, 'Aluno B NÃO deve enxergar nenhum registro do Aluno A');
