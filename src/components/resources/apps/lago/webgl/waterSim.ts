...
export class WebGLWaterSimulation {
  ...
  private createFBO(): TexturePair {
    ...
    if (status !== gl.FRAMEBUFFER_COMPLETE) {
      console.warn(
        "FBO status incomplete with current format, attempting unsigned byte fallback.",
      );
  ...
  public addDrop(
    ...
    gl.uniform2f(
      gl.getUniformLocation(this.dropProgram, "u_center"),
      normX,
      1.0 - normY,
    );
    gl.uniform1f(gl.getUniformLocation(this.dropProgram, "u_radius"), radius);
    gl.uniform1f(
      gl.getUniformLocation(this.dropProgram, "u_strength"),
      strength,
    );
  ...
  public step(damping = 0.992) {
    ...
    gl.uniform2f(
      gl.getUniformLocation(this.updateProgram, "u_delta"),
      delta,
      delta,
    );
    gl.uniform1f(
      gl.getUniformLocation(this.updateProgram, "u_damping"),
      damping,
    );
  ...
  public render(
    ...
    gl.bindTexture(gl.TEXTURE_2D, this.underwaterTexture);
    gl.uniform1i(
      gl.getUniformLocation(this.compositeProgram, "u_underwater"),
      1,
    );
    ...
    const delta = 1.0 / this.simResolution;
    gl.uniform2f(
      gl.getUniformLocation(this.compositeProgram, "u_delta"),
      delta,
      delta,
    );
    gl.uniform1f(
      gl.getUniformLocation(this.compositeProgram, "u_refraction"),
      refractionStrength,
    );
    gl.uniform1f(
      gl.getUniformLocation(this.compositeProgram, "u_specular"),
      sunlightIntensity,
    );
    gl.uniform1f(
      gl.getUniformLocation(this.compositeProgram, "u_caustics"),
      causticsIntensity,
    );
    ...
    gl.uniform1f(
      gl.getUniformLocation(this.compositeProgram, "u_ambientMode"),
      ambientCode,
    );
