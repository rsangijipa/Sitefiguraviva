  ...
  for (const fish of fishList) {
    ...
    if (!targetFound) {
      if (fish.isFrightened) {
        ...
        fish.speed =
          fish.speed * 0.98 +
          (1.2 + Math.sin(fish.wigglePhase * 0.4) * 0.4) * 0.02;
    ...
    for (let s = 1; s < spineSegments; s++) {
      ...
      const lateralFactor =
        Math.sin(fish.wigglePhase - s * 0.5) * (s * 0.9) * (fish.speed / 2.0);
...
export function renderKoiFishOnCanvas(
  ...
  for (const fish of fishList) {
    ...
    const tailAngle = Math.atan2(
      tailSeg2.y - tailSeg1.y,
      tailSeg2.x - tailSeg1.x,
    );
