...
import {
  createRiverbedCanvas,
  createSkyCanvas,
} from "../webgl/riverbedTexture";
import {
  createKoiFish,
  updateKoiFish,
  renderKoiFishOnCanvas,
} from "../simulation/koiFish";
import {
...
function applyWindDrops(
  ...
  for (let c = 0; c < cols; c++) {
    for (let r = 0; r < rows; r++) {
      ...
      const strength =
        intensity * (0.04 + Math.abs(Math.cos(phase * 1.2 + r)) * 0.06);
...
export const PondCanvas = forwardRef<PondCanvasHandle, PondCanvasProps>(
  (
    { config, onLoadProgress, onLoadComplete, onWebGLError, onAudioStatus },
    ref,
  ) => {
    ...
    const activePointersRef = useRef<
      Map<number, { lastX: number; lastY: number; lastTime: number }>
    >(new Map());
    ...
    useEffect(() => {
      ...
      onAudioStatus?.({
        available: waterAudio.getAvailable(),
        error: waterAudio.getError(),
      });
    ...
    const handleResize = useCallback(() => {
      ...
      const dpr = Math.min(
        ...
        qualitySettings[qualityRef.current].dpr,
      );
    ...
    useEffect(() => {
      ...
      const loop = (currentTime: number) => {
        ...
        foodRef.current = foodRef.current.filter(
          (f) => !f.consumed && f.life > 0,
        );
        ...
        const shouldUploadTextures =
          currentTime - lastTextureUploadRef.current >=
          qualitySettings[cfg.quality].textureCadence;
        const underCanvas = underwaterCanvasRef.current;
        const underCtx = underCanvas?.getContext("2d");
        if (
          shouldUploadTextures &&
          underCanvas &&
          underCtx &&
          riverbedCanvasRef.current
        ) {
    ...
    const drawRainOverlay = (time: number, cfg: WaterSimConfig) => {
      ...
      if (cfg.rainIntensity === "none" || rainDropsRef.current.length === 0)
        return;
      ...
      for (const drop of rainDropsRef.current) {
        ...
        const alpha =
          drop.alpha * (cfg.rainIntensity === "medium" ? 0.7 : 0.45);
      ...
      if (cfg.rainIntensity === "medium") {
        for (const drop of rainDropsRef.current) {
          if (
            drop.life > drop.maxLife * 0.7 &&
            drop.life < drop.maxLife * 0.95
          ) {
            const progress =
              (drop.life - drop.maxLife * 0.7) / (drop.maxLife * 0.25);
            const r = progress * 12 * scaleX;
            const a = (1 - progress) * 0.4;
            ctx.beginPath();
            ctx.arc(
              drop.x * scaleX,
              drop.y * scaleY + drop.length * scaleY,
              r,
              0,
              Math.PI * 2,
            );
