...
async function runTests() {
  ...
  const saveResultA = await saveRiverSession(
    {
      ...
      reflection:
        "Percebi uma sensação de tranquilidade ao ver o pensamento ir embora.",
      schema_version: 1,
  ...
  assert(
    saveResultA.success === true,
    "Aluno A deveria conseguir salvar sua sessão",
  );
  ...
  assert(
    listB.sessions.length === 0,
    "Aluno B NÃO deve visualizar sessões do Aluno A (Isolamento RLS)",
  );

  // Aluno B tenta excluir sessão de A
  const deleteByB = await deleteRiverSession(sessionIdA, userB);
  assert(
    deleteByB.success === false,
    "Aluno B NÃO pode excluir dados do Aluno A (Proteção RLS)",
  );

  // Aluno A lista sessões
  const listA = await listUserRiverSessions(userA);
  assert(
    listA.sessions.length === 1,
    "Aluno A deve encontrar seu próprio registro",
  );
  assert(
    listA.sessions[0].reflection?.includes("tranquilidade") === true,
    "Reflexão de A deve corresponder",
  );
  ...
  assert(
    saveAnon.success === false,
    "Usuário anônimo não deve salvar sessão persistente",
  );
  ...
  assert(
    exportA.data.length === 1,
    "Exportação de A deve conter apenas registros de A",
  );
  assert(
    exportA.data[0].id === sessionIdA,
    "ID exportado deve corresponder ao registro de A",
  );

  const exportB = await exportUserData(userB);
  assert(
    exportB.data.length === 0,
    "Exportação de B deve vir vazia sem contaminar dados de A",
  );
  ...
  assert(
    updateAttemptByStudent.success === false,
    "Aluno não deve conseguir alterar configurações editoriais",
  );
  ...
  assert(
    updateAttemptByAdmin.success === true,
    "Admin deve conseguir atualizar parâmetros editoriais",
  );

  const currentContent = await getPublishedResourceContent();
  assert(
    currentContent.title === "Rio dos Pensamentos (Atualizado)",
    "Conteúdo atualizado deve estar disponível",
  );
  console.log(
    " -> OK: Regras de papel e autorização docente/discente validadas.",
  );
  ...
  assert(
    scene.getLeaves().length === 8,
    "Cena deve conter exatamente 8 folhas",
  );
  ...
  assert(
    ninth.success === false,
    "Motor deve impedir inserção além do limite de 8 folhas",
  );
  assert(
    scene.getLeaves().length === 8,
    "Contagem de folhas deve permanecer em 8",
  );
  ...
  assert(
    scene.getLeaves().length === 7,
    "Folha que saiu deve ser descartada da memória imediatamente",
  );
  ...
  assert(
    duplicateSave.success === true,
    "Operação idempotente deve retornar sucesso",
  );
  const allSessionsA = await listUserRiverSessions(userA);
  assert(
    allSessionsA.sessions.length === 1,
    "Não deve criar linha duplicada para a mesma requisição",
  );
  ...
  assert(
    invalidSave.success === false,
    "Reflexão com mais de 500 caracteres deve ser rejeitada",
  );
  ...
  assert(
    sediments.length > 0,
    "Cena deve possuir partículas e sedimentos fluviais",
  );
  ...
  assert(
    deepCount > 0 && midCount > 0 && surfaceCount > 0,
    "Sedimentos devem estar distribuídos nas 3 camadas de profundidade",
  );

  const shaderLeaves = scene.getShaderLeavesData(400);
  assert(
    shaderLeaves.length === scene.getLeaves().length,
    "Shader data deve corresponder ao número de folhas ativas",
  );
  assert(
    shaderLeaves[0].scale > 0,
    "Escala da folha para shader deve ser válida",
  );
...
describe("Rio dos Pensamentos", () => {
  it("executa a suite automatizada de isolamento e integridade do rio", async () => {
    await runTests();
  });
});
