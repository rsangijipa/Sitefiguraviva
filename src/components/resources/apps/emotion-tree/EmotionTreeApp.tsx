...
import { FavoritesDrawer } from "@/components/resources/apps/emotion-tree/components/ui/FavoritesDrawer";
import { LeafMessageCard } from "@/components/resources/apps/emotion-tree/components/ui/LeafMessageCard";
import { ThemeFilter } from "@/components/resources/apps/emotion-tree/components/ui/ThemeFilter";
import { themeLabel } from "@/components/resources/apps/emotion-tree/data/labels";
import { THEMES } from "@/components/resources/apps/emotion-tree/data/themes";
import { usePerformanceMode } from "@/components/resources/apps/emotion-tree/hooks/usePerformanceMode";
import { useReducedMotionPreference } from "@/components/resources/apps/emotion-tree/hooks/useReducedMotionPreference";
import { useSessionId } from "@/components/resources/apps/emotion-tree/hooks/useSessionId";
import { useSoundscape } from "@/components/resources/apps/emotion-tree/hooks/useSoundscape";
import { fetchFavorites, postFavorite, postInteraction } from "@/components/resources/apps/emotion-tree/lib/client/interactions-api";
import { fetchQuotesByTheme } from "@/components/resources/apps/emotion-tree/lib/client/quote-api";
import { createTreeSeed, MESSAGE_LEAF_COUNT } from "@/components/resources/apps/emotion-tree/lib/theme/scene-tokens";
import { loadFavorites, mergeFavoriteIds, saveFavorites } from "@/components/resources/apps/emotion-tree/lib/utils/local-favorites";
import { INTRO_STORAGE_KEY, migrateLegacyStorage } from "@/components/resources/apps/emotion-tree/lib/utils/storage";
import type { TreeSceneApi } from "@/components/resources/apps/emotion-tree/components/3d/TreeScene";
import { useQuoteStore } from "@/components/resources/apps/emotion-tree/store/useQuoteStore";
import type { QualityProfile } from "@/components/resources/apps/emotion-tree/types/performance";
import type { Quote } from "@/components/resources/apps/emotion-tree/types/quote";

const TreeScene = dynamic(() => import("@/components/resources/apps/emotion-tree/components/3d/TreeScene"), {
  ssr: false,
  loading: () => (
...
export function EmotionTreeApp() {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape" && (favoritesOpen || panelOpen)) {
  ...
  }, [favoritesOpen, panelOpen, setPanelOpen]);
  ...
  return (
    <section aria-label="Árvore das Emoções" className="emotion-tree-app relative h-full min-h-0 w-full overflow-hidden bg-[#0D1422] text-[#EAF2FB]">
      ...
      */}
      ...
      <div className="pointer-events-none absolute inset-0 z-10">
        <div className="absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-[#0D1422]/[0.16] to-transparent" />
        <div className="absolute inset-x-0 bottom-0 h-48 bg-gradient-to-t from-[#0D1422]/[0.8] to-transparent" />
        <AnimatePresence>
          {panelOpen && activeQuote ? (
            <motion.div
              ...
              className="absolute inset-0 bg-black/[0.45] backdrop-blur-[2px]"
            />
          ) : null}
        </AnimatePresence>
      </div>
      ...
      */}
      ...
      >
        <div className="relative mx-auto h-full w-full max-w-[1240px] px-4 sm:px-6 lg:px-8">
          <motion.div
            ...
            className="pointer-events-none absolute left-3 top-3 sm:left-5 sm:top-5 lg:left-7"
          >
            {/*
                ...
                {hudExpanded ? null : (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="hud-pill hud-pill--primary pointer-events-auto inline-flex h-10 items-center gap-2 px-3 text-[11px] font-semibold backdrop-blur-md transition"
                    >
                  ...
                  >
                    <header className="flex items-start justify-between gap-3 px-4 pt-4">
                      ...
                      <button
                        ...
                        className="-mr-1 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[#93A8BE] transition hover:bg-white/[0.1] hover:text-white"
                      >
                    ...
                    <div className="flex flex-wrap items-center gap-2 px-4 pt-3.5">
                      ...
                      {showIntro ? (
                        <button
                          ...
                          className="h-9 rounded-full border border-white/[0.15] px-3.5 text-[11px] font-medium text-white/[0.75] transition hover:bg-white/[0.1] hover:text-white"
                        >
                          Explorar
                        </button>
                      ) : null}
                    ...
                    {showIntro ? (
                      <ol className="mt-3.5 ml-4 space-y-1.5 px-4 text-[11px] leading-relaxed text-[#C7D6E6]/[0.85]">
                    ...
                    <section className="px-4 pt-3">
                      <div className="mb-2 flex items-baseline justify-between gap-2">
                        ...
                        <span className="truncate text-[11px] text-white/[0.55]">{themeContextLabel}</span>
                      </div>
                    ...
                    <footer className="flex items-center justify-between px-4 py-3">
                      <button
                        ...
                        className="inline-flex h-8 items-center gap-2 rounded-full px-2.5 text-[11px] font-medium text-[#C7D6E6] transition hover:bg-white/[0.1] hover:text-white"
                      >
                        <Heart className="h-3.5 w-3.5" aria-hidden />
                        Favoritas
                        {favorites.length > 0 ? (
                          <span className="rounded-full bg-white/[0.12] px-1.5 py-px text-[10px] font-bold tabular-nums">
                            {favorites.length}
                          </span>
                        ) : null}
                      </button>

                      <span className="text-[10px] tracking-[0.12em] text-white/[0.35]">
      ...
      {!panelOpen ? (
        <button
          ...
          className="hud-pill pointer-events-auto absolute right-3 top-3 z-30 flex h-10 items-center gap-2 px-3 text-[11px] font-semibold text-[#D6E2F0] backdrop-blur-md transition hover:text-white sm:right-5 sm:top-5"
        >
      ...
      */}
      {isMobile && !panelOpen ? (
        <motion.button
          ...
          type="button"
          onClick={requestRandomLeaf}
          className="pointer-events-auto absolute bottom-[max(1.25rem,env(safe-area-inset-bottom))] left-1/2 z-30 flex h-12 -translate-x-1/2 items-center gap-2 rounded-full bg-[#F2EFE8] px-5 text-[12px] font-bold text-[#1C1A17] shadow-[0_10px_30px_rgba(0,0,0,0.45)]"
        >
