...
interface SensationButtonProps {
  type: SensationType;
  selectedSensation: SensationType | null;
  onSelect: (type: SensationType) => void;
}

const SensationButton: React.FC<SensationButtonProps> = ({
  type,
  selectedSensation,
  onSelect,
}) => {
  ...
  const isSelected = selectedSensation === type;
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-br ${config.gradient} opacity-0 ${isSelected ? "opacity-100" : "group-hover:opacity-100"} transition-opacity duration-300`}
      />
      ...
      {/* Label */}
      <span
        className={`text-xs font-bold uppercase tracking-widest relative z-10 ${isSelected ? "text-stone-800" : "text-stone-600 group-hover:text-stone-800"}`}
      >
        {config.label}
      </span>
    </button>
...
const Scanner: React.FC<ScannerProps> = ({ onComplete }) => {
  ...
  const [tempSensation, setTempSensation] = useState<SensationType | null>(
    null,
  );
      ...
      {selectedPart && (
        <div className="absolute inset-0 z-20 bg-paper/80 backdrop-blur-md flex items-end md:items-center justify-center p-4 animate-fade-in">
          <div className="relative flex max-h-[calc(100%-1rem)] w-full max-w-sm flex-col space-y-5 overflow-hidden rounded-[2rem] border border-white/50 bg-[#fafaf9] p-5 shadow-2xl sm:p-7">
            ...
            <p className="text-stone-500 text-sm font-light relative z-10">
            ...
            <div className="overflow-y-auto no-scrollbar py-1 relative z-10">
              <div className="grid grid-cols-2 gap-3">
                <SensationButton
                  type="tension"
                  selectedSensation={tempSensation}
                  onSelect={setTempSensation}
                />
                <SensationButton
                  type="heat"
                  selectedSensation={tempSensation}
            ...
            <div
              className={`space-y-4 transition-all duration-500 ease-in-out overflow-hidden relative z-10 ${tempSensation ? "opacity-100 max-h-40" : "opacity-0 max-h-0"}`}
            >
            ...
            <div className="relative z-10 pt-2">
              <button
                ...
                className={`resource-action
                  w-full rounded-xl font-medium tracking-wide transition-all duration-300
                  ${
                    tempSensation
      ...
      {!selectedPart && (
        <div className="absolute bottom-8 left-0 right-0 text-center pointer-events-none">
          <p className="text-stone-500 text-sm font-serif italic bg-white/70 inline-block px-6 py-3 rounded-full backdrop-blur-sm border border-white shadow-sm transition-all duration-500">
            {isPlaying
              ? "Ouça o guia. Toque no corpo para registrar sensações."
              : "Pausado. Toque no play para continuar."}
          </p>
        </div>
      )}
    </div>
