...
describe("Sons para Awareness", () => {
  it("valida manifesto acustico, cenas guiadas, esquemas e RLS", async () => {
    ...
    requiredSounds.forEach((id) => {
      ...
      assert(
        s.license.includes("Creative Commons") || s.license.includes("CC"),
        `Licença declarada para ${s.title}`,
      );
      assert(
        s.textualDescription.length > 20,
        `Descrição textual acessível para ${s.title}`,
      );
      assert(s.mimeType === "audio/wav", `MIME type válido para ${s.title}`);
      assert(
        s.durationSeconds >= 60,
        `Duração suficiente de loop (>=60s) para ${s.title}`,
      );
    });
    ...
    GUIDED_SCENES.forEach((scene, i) => {
      assert(
        !scene.guidanceNote.includes("correto") &&
          !scene.guidanceNote.includes("erro"),
        `Cena ${i + 1} não contém julgamento de erro/acerto`,
      );
      assert(
        scene.configuredPosition.x >= -1 && scene.configuredPosition.x <= 1,
        `Cena ${i + 1} possui X normalizado`,
      );
      assert(
        scene.configuredPosition.y >= -1 && scene.configuredPosition.y <= 1,
        `Cena ${i + 1} possui Y normalizado`,
      );
    });
    ...
    assert(
      !tooManyQualities.isValid,
      "Rejeita observação com mais de 5 qualidades",
    );
    ...
    assert(
      !invalidTerm.isValid,
      "Rejeita termo próprio com mais de 120 caracteres",
    );

    // Reflexão pessoal ≤ 500 caracteres
    const validReflect = validateReflection(
      "Uma prática tranquila de escuta centrada.",
    );
    assert(validReflect.isValid, "Reflexão de tamanho adequado aceita");

    const invalidReflect = validateReflection("B".repeat(501));
    assert(
      !invalidReflect.isValid,
      "Rejeita reflexão excedendo 500 caracteres",
    );
    ...
    async function testRls() {
      ...
      assert(
        entryA.userId === "user_A",
        "Registro associado corretamente ao usuário autenticado A",
      );
      ...
      assert(
        entriesB.length === 0,
        "RLS ativa: Usuário B não consegue visualizar dados de Usuário A",
      );
      ...
      assert(
        deleteBlocked,
        "RLS ativa: Usuário B impedido de excluir registro pertencente a Usuário A",
      );
      ...
      assert(
        anonBlocked,
        "Sessões anônimas não podem salvar no histórico privado permanente",
      );
    }
