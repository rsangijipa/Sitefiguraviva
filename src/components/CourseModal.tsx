          ...
          <div className="p-8 md:p-12">
            <div className="grid lg:grid-cols-2 gap-12 items-start">
              ...
              <div className="space-y-8">
                ...
                <div>
                  <h1 className="font-serif text-3xl md:text-5xl text-primary leading-tight mb-4">
                    {course.title}
                  </h1>
                  {course.subtitle && (
                    <p className="text-xl font-serif italic text-gold/80 leading-snug">
                      {course.subtitle}
                    </p>
                  )}
                </div>
                  ...
                  course.mediators.length > 0 && (
                    <div className="border-l-4 border-accent/20 pl-6 py-2">
                      ...
                      <div className="flex flex-wrap gap-4">
                        {course.mediators.map(
                          (mediator: any, index: number) => {
                            ...
                            return (
                              ...
                              >
                                <div className="w-10 h-10 rounded-full bg-stone-100 overflow-hidden shrink-0 border border-stone-200 relative">
                                  {details.image ? (
                                    <Image
                                      src={details.image}
                                      alt={details.name}
                ...
                <button
                  onClick={() => {
                    onClose();
                    window.location.href = `/inscricao/${course.id}`;
                  }}
                ...
                <div className="md:col-span-4">
                  {course.details?.schedule && (
                    <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm">
                      <h4 className="font-bold text-primary text-xs uppercase tracking-widest mb-6">
                        Cronograma
                      </h4>
                        ...
                        {Array.isArray(course.details.schedule) ? (
                          course.details.schedule.map(
                            (date: string, i: number) => (
                              <div
                                key={i}
                                className="bg-stone-50 px-3 py-2 rounded text-center"
                              >
                          ...
                          <p className="text-xs text-primary/70 whitespace-pre-line">
                            {course.details.schedule}
                          </p>
                        )}
                      </div>
          ...
          {selectedMediator && (
            <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                className="bg-white max-w-xl w-full rounded-[2.5rem] relative z-10 shadow-2xl p-10 overflow-hidden"
              >
                <button
                  onClick={() => setSelectedMediator(null)}
                  className="absolute top-6 right-6 p-2 rounded-full hover:bg-stone-100 text-stone-400 hover:text-primary transition-all active:scale-90"
                >
                  <X size={20} />
                </button>

                <div className="flex flex-col items-center text-center">
                  <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-stone-50 shadow-elev-3 mb-8 relative">
