...
export function CertificateIssueCard({ courseId, courseTitle, progressPercent, isCompleted, existingCertificate }: CertificateIssueCardProps) {
        ...
        return (
            <div className="bg-stone-50 border border-stone-200 rounded-xl p-6 flex items-center gap-4 opacity-75">
                ...
                <div>
                    ...
                    <p className="text-sm text-stone-500">Complete 100% do curso para liberar.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className={cn("border rounded-xl p-6 flex flex-col md:flex-row items-center gap-6 shadow-sm transition-all",
            issued ? "bg-green-50 border-green-200" : "bg-gradient-to-br from-yellow-50 to-amber-50 border-amber-200")}>

            <div className={cn("w-16 h-16 rounded-full flex items-center justify-center shrink-0",
                issued ? "bg-green-100 text-green-600" : "bg-amber-100 text-amber-600")}>
                <Award size={32} />
            </div>

            <div className="flex-1 text-center md:text-left">
                <h3 className={cn("font-bold text-lg", issued ? "text-green-900" : "text-amber-900")}>
                    {issued ? "Certificado Emitido" : "Certificado de Conclusão"}
                </h3>
                <p className={cn("text-sm", issued ? "text-green-800/80" : "text-amber-800/80")}>
                    {issued
                        ? "Seu certificado já está disponível para download e validação."
                        : <>Parabéns! Você concluiu <strong>{courseTitle}</strong> e seu certificado já pode ser emitido.</>}
                </p>
            </div>
            ...
            {issued && certId ? (
                ...
                >
                    {loading ? <Loader2 className="animate-spin" /> : <Award className="mr-2" size={18} />}
                    Emitir Agora
                </Button>
            )}
        </div>
