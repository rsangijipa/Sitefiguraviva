 ...
 */
export async function activateEnrollmentFromStripe(payload: StripeActivationPayload) {
    const { uid, courseId, sessionId, isSubscription, accessUntil, paymentStatus = 'paid' } = payload;
    ...
    await logAudit({
        ...
        action: isSubscription ? 'SUBSCRIPTION_ACTIVATED' : 'STRIPE_PAYMENT_CONFIRMED',
        target: { collection: 'enrollments', id: enrollmentId, summary: `Stripe activation for ${uid}` },
 ...
 */
export async function expireSubscriptionEnrollment(uid: string, courseId: string) {
    const enrollmentId = `${uid}_${courseId}`;
    const enrollmentRef = adminDb.collection('enrollments').doc(enrollmentId);
    const userEnrollmentRef = adminDb.collection('users').doc(uid).collection('enrollments').doc(courseId);
    ...
    await logAudit({
        ...
        target: { collection: 'enrollments', id: enrollmentId, summary: `Subscription expired for ${uid}` },
        metadata: { courseId }
    });
}
