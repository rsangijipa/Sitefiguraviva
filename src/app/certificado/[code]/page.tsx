...
export default async function CertificateValidationPage({ params }: Props) {
  ...
  const { data: cert, error } = await createSupabaseServiceClient()
    .from("certificates")
    .select("*")
      ...
      <div className="min-h-screen bg-stone-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-red-100">
          ...
          <p className="text-stone-500 mb-6">
            O código <strong>{code}</strong> não foi encontrado em nossa base de
            registros.
          </p>
          <Link
            href="/"
        ...
        <div className="container mx-auto px-4 flex justify-between items-center">
          <Link href="/" className="flex items-center gap-2">
            <div className="font-serif text-xl text-primary font-bold">
              Figura <span className="text-amber-500 italic">Viva</span>
            </div>
          </Link>
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <div className="bg-white p-8 md:p-12 rounded-2xl shadow-2xl border border-amber-100 max-w-2xl w-full text-center relative overflow-hidden">
          ...
          <p className="text-stone-500 mb-8">
            Este documento foi emitido oficialmente pelo Instituto Figura Viva.
          </p>
          ...
          <div className="bg-stone-50 rounded-xl p-8 border border-stone-100 mb-8 text-left space-y-4">
            ...
            <div>
              ...
              <span className="text-xl font-bold text-primary">
                {metadata.courseTitle || "Curso não identificado"}
              </span>
            </div>
            <div className="flex gap-8">
              ...
              <div>
                ...
                <span className="font-mono bg-stone-200 px-2 py-1 rounded text-stone-600 text-sm">
                  {cert.code}
                </span>
              </div>
            </div>
          ...
          <p className="text-xs text-stone-400 max-w-sm mx-auto">
            O Instituto Figura Viva certifica que o aluno acima cumpriu todos os
