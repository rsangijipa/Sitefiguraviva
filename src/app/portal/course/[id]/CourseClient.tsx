...
function CourseContent({ initialData }: { initialData?: any }) {
    ...
    const hasFreeAccess = modules.some(m => m.lessons.some(l => (l as any).isFreePreview)); // Cast access if type not updated yet

    if (status !== 'active' && !isLoading && !hasFreeAccess) {
