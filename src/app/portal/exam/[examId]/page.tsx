...
import { AssessmentPlayer } from '@/components/assessment/AssessmentPlayer'; // Ensure this path is correct
import { CheckCircle, AlertTriangle, ArrowLeft } from 'lucide-react';
import Link from 'next/link';
import Button from '@/components/ui/Button';

// Wrapper component to handle client-side navigation (Exit)
import { ExamExitWrapper } from '@/components/assessment/ExamExitWrapper'; // We'll create this small wrapper or inline logic
...
export default async function ExamPage({ params }: { params: Promise<{ examId: string }> }) {
    ...
    if (!assessmentDoc.exists) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
                ...
                <h1 className="text-2xl font-bold text-stone-800">Avaliação não encontrada</h1>
                <Link href="/portal" className="mt-4 text-primary font-bold hover:underline">Voltar ao Painel</Link>
            </div>
        );
    }

    const assessment = { id: assessmentDoc.id, ...assessmentDoc.data() } as Assessment;

    // Check if already completed
    if (!submissionsSnap.empty) {
        const submission = { id: submissionsSnap.docs[0].id, ...submissionsSnap.docs[0].data() } as Submission;
            ...
            <div className="min-h-screen bg-[#FDFCF9] flex items-center justify-center p-6">
                <div className="max-w-md w-full bg-white rounded-2xl shadow-xl border border-stone-100 p-8 text-center animate-fade-in-up">
                    ...
                    <p className="text-stone-500 mb-6">
                        Você finalizou a avaliação <strong>{assessment.title}</strong> com nota <strong>{submission.grade?.toFixed(1)}</strong>.
                    </p>
    ...
    // Check for Draft
    const draftDoc = await db.collection('submissions').doc(`${uid}_${examId}_draft`).get();
    const existingSubmission = draftDoc.exists ? { id: draftDoc.id, ...draftDoc.data() } as Submission : undefined;

    return (
        <ExamExitWrapper assessment={assessment} existingSubmission={existingSubmission} />
    );
}
