...
export default async function AdminAssessmentsPage() {
    ...
    // Fetch Assessments
    const assessmentsSnap = await db.collection('assessments').orderBy('title').get();
    ...
    return (
        <div className="space-y-6">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-stone-800">Avaliações & Provas</h1>
                    <p className="text-stone-500 text-sm">Gerencie o conteúdo das provas de todos os cursos.</p>
                </div>
                <div className="flex gap-2">
                    <Button variant="primary" className="shadow-lg shadow-primary/20" leftIcon={<Plus size={18} />}>
                        Nova Avaliação
                    </Button>
                </div>
            </header>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-stone-50 border-b border-stone-200">
                            <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-stone-400">Título</th>
                            <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-stone-400">Curso / Módulo</th>
                            <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-stone-400">Questões</th>
                            <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-stone-400">Média Mín.</th>
