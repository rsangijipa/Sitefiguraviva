...
export default function GalleryManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await saveGalleryItemAction(formData.id || null, formData);
      if (res.success) {
        addToast(
          formData.id ? "Imagem atualizada!" : "Imagem salva com sucesso!",
          "success",
        );
        refetch();
      ...
      addToast("Erro ao salvar item: " + error.message, "error");
    } finally {
  ...
  return (
    <div className="space-y-12 pb-20">
              ...
              >
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-gray-200 rounded-2xl px-6 py-4 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all text-primary font-serif text-lg"
                    placeholder="Ex: Workshop de Gestalt"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full bg-white border border-gray-200 rounded-2xl px-6 py-4 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all text-sm font-bold text-primary appearance-none cursor-pointer"
                  >
                    <option value="Geral">Geral</option>
                    <option value="Eventos">Eventos</option>
                    <option value="Workshops">Workshops</option>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full bg-white border border-gray-200 rounded-2xl px-6 py-4 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all text-sm text-primary/80"
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-gray-200 rounded-2xl px-6 py-4 focus:outline-none focus:ring-2 focus:ring-gold/20 focus:border-gold transition-all text-sm text-primary/60"
                ...
                <div className="pt-4 flex justify-end gap-4 border-t border-gray-100">
                  <button
                    type="button"
                    onClick={() => setIsEditing(false)}
                    className="px-8 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest text-primary/40 hover:bg-gray-50 transition-colors"
                  ...
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className="bg-primary text-paper px-8 py-3 rounded-xl flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest hover:bg-gold transition-soft shadow-lg disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    <Save size={16} />{" "}
      ...
      {status.message && (
        <div
          className={`fixed bottom-8 right-8 z-50 p-4 rounded-xl flex items-center gap-3 text-sm font-medium shadow-2xl animate-in slide-in-from-right-10 duration-500 ${status.type === "error" ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600"}`}
        >
          {status.type === "error" ? (
            <AlertCircle size={18} />
          ) : (
