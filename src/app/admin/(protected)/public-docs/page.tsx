...
export default function DocumentManager() {
  ...
  const loadDocuments = async () => {
    try {
      setDocuments(await listAdminPublicDocuments());
    } catch (error) {
      console.warn("[DocumentManager] Failed to load documents:", error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await createPublicDocument(formData);
  ...
  const handleDelete = async (docObj: any) => {
    ...
    try {
      const result = await deletePublicDocument(docObj.id);
      if (result.error) throw new Error(result.error);
