...
import {
  createPublicDocumentAction,
  deletePublicDocumentAction,
  listPublicDocumentsAction,
} from "@/app/actions/public-documents";
...
export default function DocumentManager() {
  ...
  const loadDocuments = async () => {
    try {
      setDocuments(await listPublicDocumentsAction());
    } catch (error) {
      console.error("[DocumentManager] Failed to load documents:", error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createPublicDocumentAction(formData);
  ...
  const handleDelete = async (docObj: any) => {
    ...
    try {
      await deletePublicDocumentAction(docObj.id);
    ...
    <div className="space-y-12 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredDocuments.map((doc: any) => (
          <div
            key={doc.id}
            className="bg-white p-6 rounded-2xl border border-stone-100 shadow-sm hover:shadow-lg transition-all group relative"
          >
      ...
      <Modal
        ...
        ariaLabel="Adicionar documento público"
      >
