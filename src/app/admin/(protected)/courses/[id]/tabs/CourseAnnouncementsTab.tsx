...
export default function CourseAnnouncementsTab({
  ...
  if (loading)
    return (
      <div className="p-8 text-center text-stone-400">Carregando avisos...</div>
    );
    ...
    <div className="space-y-6 animate-in fade-in">
      <div className="bg-blue-50 p-4 rounded-xl flex items-start gap-4 text-blue-800 border border-blue-200">
        ...
        <div>
          ...
          <p className="text-xs opacity-80 mt-1">
            Crie comunicados importantes para os alunos. Eles aparecerão no
            painel do curso.
          </p>
        </div>
      </div>
      ...
      {!isCreating ? (
        ...
        <form
          ...
          className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm space-y-4"
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, publishAtDate: e.target.value })
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, publishAtTime: e.target.value })
                }
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="isPinned" className="text-sm text-stone-700">
              Fixar no topo
            </label>
          </div>
      ...
      <div className="space-y-4">
        {announcements.map((item) => (
          <div
            key={item.id}
            className="bg-white p-4 rounded-xl border border-stone-200 flex items-start justify-between group"
          >
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mt-2 text-xs text-stone-400">
                <span className="flex items-center gap-1">
                  <Calendar size={12} />{" "}
                  {item.publishAt
                    ? (item.publishAt as any).toDate
                      ? (item.publishAt as any).toDate().toLocaleString()
                      : new Date(item.publishAt as any).toLocaleString()
                    : "N/A"}
                </span>
                {item.publishAt &&
                  ((item.publishAt as any).toDate
                    ? (item.publishAt as any).toDate()
                    : new Date(item.publishAt as any)) > new Date() && (
                    <span className="text-orange-500 font-bold">Agendado</span>
                  )}
              </div>
            </div>
            <button
