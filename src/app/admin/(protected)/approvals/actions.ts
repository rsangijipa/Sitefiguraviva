...
export async function approveEnrollment(enrollmentId: string, uid: string, courseId: string, currentData: any) {
    try {
        ...
        await logAudit({
            ...
            target: { collection: 'enrollments', id: enrollmentId, summary: `Approved access for user ${uid} to course ${courseId}` },
        ...
        return { success: false, error: error.message || 'Erro ao aprovar matrícula.' };
    }
}

export async function rejectEnrollment(enrollmentId: string, uid: string, courseId: string, reason: string) {
    try {
        ...
        const batch = adminDb.batch();
        batch.update(enrollmentRef, updateData);
        batch.update(adminDb.collection('users').doc(uid).collection('enrollments').doc(courseId), updateData);
        ...
        await logAudit({
            ...
            target: { collection: 'enrollments', id: enrollmentId, summary: `Rejected access for user ${uid} to course ${courseId}` },
        ...
        return { success: false, error: error.message || 'Erro ao rejeitar matrícula.' };
    }
}
