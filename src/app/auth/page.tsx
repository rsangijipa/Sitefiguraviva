...
const getFriendlyErrorMessage = (code: string) => {
    switch (code) {
        ...
        case 'auth/cancelled-popup-request': return null; // Ignore overlap
        case 'auth/account-exists-with-different-credential': return 'Conta já existe com outro método de login.';
        default: return 'Ocorreu um erro. Tente novamente.';
    }
};

function AuthContent() {
    const { signIn, signUp, updateProfile } = useAuth(); // Context needs to be able to handle this? Or usage assumes standard firebase auth context.
    // Assuming AuthContext exposes these. If not, we might need to use direct firebase functions, but existing login page used useAuth.
    // Existing page used: const { signIn, signUp, updateProfile } = useAuth();
    ...
    const handleSuccess = async (user: any) => {
        try {
            ...
            // C. Handle Intent (Enrollment)
            if (intent === 'enroll' && (selectedCourse || courseId)) {
                // Future integration point for enrollment
            }

            // D. Redirect Logic
            let targetPath = getDashboardPathForRole(userRole);
            ...
            if (next && next.startsWith('/') && !next.startsWith('//')) {
                ...
                if (next.startsWith('/admin') && userRole !== 'admin') {
                    console.warn(`Redirect blocked: User ${user.uid} (role: ${userRole}) tried to access ${next}`);
                    targetPath = getDashboardPathForRole(userRole); // Fallback to their dashboard
                } else {
                    targetPath = next;
                }
            }
    ...
    const handleEmailAuth = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isSignup) {
                ...
                if (fullName) {
                    // We try to update profile on client too for immediate feedback
                    // updateProfile from context might wrapper user.updateProfile
                }
            } else {
    ...
    const handleGoogleAuth = async () => {
        setError('');
        setLoading(true); // Will likely redirect or popup
        ...
        if (user) {
            ...
            setLoading(false); // Cancelled or redirecting
        }
    };
    ...
    return (
        <PageShell variant="auth" className="p-4 md:p-8 min-h-screen">
            {/* Back Link */}
            <Link href="/" className="absolute top-8 left-8 z-50 flex items-center gap-2 text-primary/40 hover:text-primary transition-colors font-bold uppercase tracking-widest text-xs">
                ...
                >
                    ...
                    <div className="flex flex-col items-center mb-8">
                        <div className="w-20 h-20 rounded-full mb-6 flex items-center justify-center p-1 border border-primary/10 bg-white shadow-soft-md">
                            {/* Ensure asset exists, fallback gracefully */}
                    ...
                    <div className="relative mb-6">
                        ...
                        <div className="relative flex justify-center text-xs uppercase"><span className="bg-white px-2 text-stone-400">Ou</span></div>
                    </div>
                        ...
                        <AnimatePresence mode='wait'>
                            {isSignup && (
                                ...
                                >
                                    ...
                                    <Input label="Telefone (WhatsApp)" type="tel" value={phone} onChange={e => setPhone(e.target.value)} required placeholder="(00) 00000-0000" />

                                    {/* Course Selection (If intents require it, simplified for Unified Auth) */}
                                    {courses.length > 0 && (
                                        <div className="space-y-1">
                                            <label className="text-[10px] uppercase font-bold text-stone-500 ml-1">Interesse</label>
                        ...
                        <div className="space-y-1">
                            ...
                            {!isSignup && (
                                <div className="text-right">
                                    <Link href="/auth/reset-password" className="text-[10px] uppercase font-bold text-primary/60 hover:text-primary">
                                        Esqueceu a senha?
                                    </Link>
                                </div>
                            )}
                    ...
                    <div className="mt-8 text-center">
                        <Link href="/admin" className="text-[10px] text-stone-400/50 hover:text-stone-500 transition-all uppercase tracking-widest font-medium">
                            Admin
                        </Link>
                    </div>
