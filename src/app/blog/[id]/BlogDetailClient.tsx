    ...
    <div className="bg-paper min-h-screen">
      ...
      <main className="pt-40 pb-20">
        <article className="container mx-auto px-6 max-w-4xl">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 text-accent font-bold uppercase tracking-widest text-[10px] mb-12 hover:translate-x-[-5px] transition-transform"
          >
            <ArrowLeft size={14} /> Voltar ao Diário
          </button>

          <header className="mb-16">
            <span className="text-gold font-bold uppercase tracking-[0.3em] text-[10px] mb-6 block">
              {post.date}
            </span>
            <h1 className="font-serif text-5xl md:text-7xl text-primary leading-[1.1] mb-10">
              {post.title}
            </h1>
          ...
          <footer className="mt-24 pt-12 border-t border-primary/5">
            <div className="flex items-center gap-6">
              <div className="w-16 h-16 rounded-full bg-stone-100 overflow-hidden border border-stone-200">
                <img
                  src={getImageSrc(post.authorImage, "/assets/logo.jpeg")}
                  alt="Author"
